...
class TableCell extends Block
{
    ...
    function render(Frame $frame)
    {
        ...
        $getNumRows = $cellmap->get_getNumRows();
        ...
        if (in_array($getNumRows - 1, $cells["rows"])) {
            $draw_bottom = true;
            $bottom_row = $cellmap->get_row($getNumRows - 1);
        } else {
            $draw_bottom = false;
        }
        ...
        foreach ($cells["columns"] as $j) {
            ...
            if ($draw_bottom) {
                $bp = $cellmap->get_border_properties($getNumRows - 1, $j);
