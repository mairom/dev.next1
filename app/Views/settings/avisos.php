<?php if (check_privilege('settings', 'edit')) { ?>
    <md-content class="md-padding bg-white">
        <md-button ng-click="addAviso()" class="md-raised md-primary pull-right successButton">
            <span><?= lang2('create') . ' aviso' ?></span>
        </md-button>
    </md-content>
<?php } ?>




<md-content class="md-padding bg-white">
    <?php
    if ($user_data['admin'] == "1" || $user_data['super_admin'] == "1") {
    ?>
        <h4 style = "margin-bottom: 10px;">Servirá de modelo para novas empresas</h4>
    <?php
    }
    ?>

    <md-table-container>
        <table md-table md-progress="promise">
            <thead md-head>
                <tr md-row>
                    <th md-column width="10%">#</th>
                    <th md-column><?= lang2('name'); ?></th>
                    <th md-column><?php echo lang2('action'); ?></th>
                </tr>
            </thead>
            <tbody md-body>
                <tr class="select_row" md-row ng-repeat="aviso in avisos">
                    <td md-cell>
                        <span ng-bind="aviso.id_aviso"></span>
                    </td>
                    <td md-cell>
                        <span ng-bind="aviso.nm_aviso"></span>
                    </td>
                    <td md-cell>
                        <?php if (check_privilege('settings', 'edit')) { ?>
                            <span ng-click="addAviso(aviso)">
                                <md-icon ng-hide="editLoader == true" md-menu-align-target class="md-raised md-primary mdi mdi-edit" style="margin: auto 3px auto 0;">
                                </md-icon>
                            </span>
                        <?php }
                        if (check_privilege('settings', 'delete')) { ?>
                            <span ng-click="dellAviso(aviso.id_aviso)">
                                <md-icon md-menu-align-target class="md-raised md-primary ion-trash-b" style="margin: auto 3px auto 0;">
                                </md-icon>
                            </span>
                        <?php } ?>
                    </td>
                </tr>
            </tbody>
        </table>
    </md-table-container>
</md-content>
<br />
