...
class Login extends CI_Controller {
	...
	function index() {
		...
		return redirect()->to( 'area/panel' );

		} else {

		return redirect()->to( 'area/login/auth' );
	...
	function auth() {
		...
		if ( $this->form_validation->run() == false ) {
			...
			return redirect()->to( 'area/panel' );
				...
				return view( 'area/login/login', $data );
	...
	public function forgot() {
		...
		//	$clean = $this->security->xss_clean( $email );
			...
			$userInfo = $this->Contacts_Model->getUserInfoByEmail( $email );



			if ( !$userInfo ) {

				session()->setFlashdata( 'ntf4', lang( 'customercanffindmail' ) );

			return redirect()->to( site_url() . 'area/login' );
			...
			$this->Emails_Model->send_email($clean, $template['from_name'], $subject, $message);

			

			session()->setFlashdata( 'ntf1', '<b>' . lang( 'customerpasswordsend' ) . '</b>' );

		return redirect()->to( 'area/login' );
	...
	//	$cleanToken = $this->security->xss_clean( $token );
		...
		$user_info = $this->Contacts_Model->isTokenValid( $token ); //either false or array();               



		if ( !$user_info ) {

			session()->setFlashdata( 'ntf1', lang( 'tokenexpired' ) );
		...
		if ( $this->form_validation->run() == FALSE ) {
			...
			return view( 'area/login/reset_password', $data );

		} else {



			$post = request()->getPost( NULL, TRUE );
			...
			if ( !$this->Contacts_Model->updatePassword( $cleanPost ) ) {
				...
				session()->setFlashdata( 'ntf1', lang( 'problemupdatepassword' ) );

			} else {

				session()->setFlashdata( 'ntf1', lang( 'passwordupdated' ) );
				...
				$this->Emails_Model->send_email($user_info->email, $template['from_name'], $subject, $message);
