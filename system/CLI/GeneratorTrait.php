...
trait GeneratorTrait
{
    ...
    private function generateFile(string $target, string $content): void
    {
        if ($this->getOption('namespace') === 'CodeIgniter') {
            ...
            CLI::write(lang2('CLI.generator.usingCINamespace'), 'yellow');
            CLI::newLine();
            ...
            if (
                ...
                CLI::newLine();
                CLI::write(lang2('CLI.generator.cancelOperation'), 'yellow');
                CLI::newLine();
        ...
        if (! $this->getOption('force') && $isFile) {
            CLI::error(
                lang2('CLI.generator.fileExist', [clean_path($target)]),
                'light_gray',
                'red'
            );
            CLI::newLine();
        ...
        if (! write_file($target, $content)) {
            // @codeCoverageIgnoreStart
            CLI::error(
                lang2('CLI.generator.fileError', [clean_path($target)]),
                'light_gray',
                'red'
            );
            CLI::newLine();
        ...
        if ($this->getOption('force') && $isFile) {
            CLI::write(
                lang2('CLI.generator.fileOverwrite', [clean_path($target)]),
                'yellow'
            );
            CLI::newLine();
        ...
        CLI::write(
            lang2('CLI.generator.fileCreate', [clean_path($target)]),
            'green'
        );
        CLI::newLine();
    }
    ...
    protected function buildPath(string $class): string
    {
        ...
        if (! $base = reset($base)) {
            CLI::error(
                lang2('CLI.namespaceNotDefined', [$namespace]),
                'light_gray',
                'red'
            );
            CLI::newLine();
