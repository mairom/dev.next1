...
class Email
{
    ...
    public function attach($file, $disposition = '', $newname = null, $mime = '')
    {
        if ($mime === '') {
            if (! str_contains($file, '://') && ! is_file($file)) {
                $this->setErrorMessage(lang2('Email.attachmentMissing', [$file]));

                return false;
            }

            if (! $fp = @fopen($file, 'rb')) {
                $this->setErrorMessage(lang2('Email.attachmentUnreadable', [$file]));
    ...
    {
        if (! is_array($email)) {
            $this->setErrorMessage(lang2('Email.mustBeArray'));
        ...
        foreach ($email as $val) {
            if (! $this->isValidEmail($val)) {
                $this->setErrorMessage(lang2('Email.invalidAddress', [$val]));
    ...
    public function send($autoClear = true)
    {
        ...
        if (! isset($this->headers['From'])) {
            $this->setErrorMessage(lang2('Email.noFrom'));
        ...
        if (
            ...
            $this->setErrorMessage(lang2('Email.noRecipients'));
    ...
    protected function spoolEmail()
    {
        ...
        if (! $success) {
            $message = lang2('Email.sendFailure' . ($protocol === 'mail' ? 'PHPMail' : ucfirst($protocol)));
        ...
        $this->setErrorMessage(lang2('Email.sent', [$protocol]));
    ...
    protected function sendWithSendmail()
    {
        ...
        if ($status !== 0) {
            $this->setErrorMessage(lang2('Email.exitStatus', [$status]));
            $this->setErrorMessage(lang2('Email.noSocket'));
    ...
    {
        if ($this->SMTPHost === '') {
            $this->setErrorMessage(lang2('Email.noHostname'));
        ...
        if (! str_starts_with($reply, '250')) {
            $this->setErrorMessage(lang2('Email.SMTPError', [$reply]));
    ...
    protected function SMTPConnect()
    {
        ...
        if (! is_resource($this->SMTPConnect)) {
            $this->setErrorMessage(lang2('Email.SMTPError', [$errno . ' ' . $errstr]));
        ...
        if ($this->SMTPCrypto === 'tls') {
            ...
            if ($crypto !== true) {
                $this->setErrorMessage(lang2('Email.SMTPError', [$this->getSMTPData()]));
    ...
    protected function SMTPAuthenticate()
    {
        ...
        if ($this->SMTPUser === '' && $this->SMTPPass === '') {
            $this->setErrorMessage(lang2('Email.noSMTPAuth'));
        ...
        if (! str_starts_with($reply, '334')) {
            $this->setErrorMessage(lang2('Email.failedSMTPLogin', [$reply]));
        ...
        if (! str_starts_with($reply, '334')) {
            $this->setErrorMessage(lang2('Email.SMTPAuthUsername', [$reply]));
        ...
        if (! str_starts_with($reply, '235')) {
            $this->setErrorMessage(lang2('Email.SMTPAuthPassword', [$reply]));
    ...
    protected function sendData($data)
    {
        ...
        if (! is_int($result)) {
            $this->setErrorMessage(lang2('Email.SMTPDataFailure', [$data]));
