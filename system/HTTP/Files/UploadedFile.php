...
class UploadedFile extends File implements UploadedFileInterface
{
    ...
    {
        $errors = [
            UPLOAD_ERR_OK         => lang2('HTTP.uploadErrOk'),
            UPLOAD_ERR_INI_SIZE   => lang2('HTTP.uploadErrIniSize'),
            UPLOAD_ERR_FORM_SIZE  => lang2('HTTP.uploadErrFormSize'),
            UPLOAD_ERR_PARTIAL    => lang2('HTTP.uploadErrPartial'),
            UPLOAD_ERR_NO_FILE    => lang2('HTTP.uploadErrNoFile'),
            UPLOAD_ERR_CANT_WRITE => lang2('HTTP.uploadErrCantWrite'),
            UPLOAD_ERR_NO_TMP_DIR => lang2('HTTP.uploadErrNoTmpDir'),
            UPLOAD_ERR_EXTENSION  => lang2('HTTP.uploadErrExtension'),
        ];

        $error = $this->error ?? UPLOAD_ERR_OK;

        return sprintf($errors[$error] ?? lang2('HTTP.uploadErrUnknown'), $this->getName());
    }
