...
class TimeDifference
{
    ...
    public function humanize(?string $locale = null): string
    {
        ...
        if ($years !== 0) {
            $phrase = lang2('Time.years', [abs($years)], $locale);
            $before = $years < 0;
        } elseif ($months !== 0) {
            $phrase = lang2('Time.months', [abs($months)], $locale);
            $before = $months < 0;
        } elseif ($days !== 0 && (abs($days) >= 7)) {
            $weeks  = ceil($days / 7);
            $phrase = lang2('Time.weeks', [abs($weeks)], $locale);
            $before = $days < 0;
        } elseif ($days !== 0) {
            $phrase = lang2('Time.days', [abs($days)], $locale);
            $before = $days < 0;
        } elseif ($hours !== 0) {
            $phrase = lang2('Time.hours', [abs($hours)], $locale);
            $before = $hours < 0;
        } elseif ($minutes !== 0) {
            $phrase = lang2('Time.minutes', [abs($minutes)], $locale);
            $before = $minutes < 0;
        } else {
            return lang2('Time.now', [], $locale);
        }

        return $before
            ? lang2('Time.ago', [$phrase], $locale)
            : lang2('Time.inFuture', [$phrase], $locale);
    }
