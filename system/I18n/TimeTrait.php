...
trait TimeTrait
{
    ...
    public function humanize()
    {
        ...
        if ($years !== 0) {
            $phrase = lang2('Time.years', [abs($years)]);
            $before = $years < 0;
        } elseif ($months !== 0) {
            $phrase = lang2('Time.months', [abs($months)]);
            $before = $months < 0;
        } elseif ($days !== 0 && (abs($days) >= 7)) {
            $weeks  = ceil($days / 7);
            $phrase = lang2('Time.weeks', [abs($weeks)]);
            $before = $days < 0;
        } elseif ($days !== 0) {
            $before = $days < 0;
            ...
            if (abs($days) === 1) {
                return $before ? lang2('Time.yesterday') : lang2('Time.tomorrow');
            }

            $phrase = lang2('Time.days', [abs($days)]);
        } elseif ($hours !== 0) {
            $phrase = lang2('Time.hours', [abs($hours)]);
            $before = $hours < 0;
        } elseif ($minutes !== 0) {
            $phrase = lang2('Time.minutes', [abs($minutes)]);
            $before = $minutes < 0;
        } else {
            return lang2('Time.now');
        }

        return $before ? lang2('Time.ago', [$phrase]) : lang2('Time.inFuture', [$phrase]);
    }
