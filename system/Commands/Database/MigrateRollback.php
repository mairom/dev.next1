...
class MigrateRollback extends BaseCommand
{
    ...
    public function run(array $params)
    {
        if (ENVIRONMENT === 'production') {
            ...
            if (! $force && CLI::prompt(lang2('Migrations.rollBackConfirm'), ['y', 'n']) === 'n') {
                return;
            }
        ...
        try {
            ...
            CLI::write(lang2('Migrations.rollingBack') . ' ' . $batch, 'yellow');

            if (! $runner->regress($batch)) {
                CLI::error(lang2('Migrations.generalFault'), 'light_gray', 'red'); // @codeCoverageIgnore
            }
